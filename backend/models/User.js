...
class User {
  ...
  static async update(body, user_id) {
    ...
    queryText += ` WHERE id = $${queryValues.length} returning id, email, first_name, full_name, alarm_status, alarm_time, alarm_days, sound_effects, auth_provider, verified`;
  ...
  static async getUser(id, email) {
    const getUser = await db.oneOrNone(
      `SELECT id, email, first_name, full_name, alarm_status, alarm_time, alarm_days, sound_effects, auth_provider, verified, created_at FROM users
